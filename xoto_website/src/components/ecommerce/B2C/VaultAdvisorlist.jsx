...
const VaultAdvisorlist = () => {
  ...
  const fetchAdvisors = useCallback(async (page = 1, limit = 10, tab = activeTab, searchTerm = search, dept = department) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.message || "Failed to load advisors", "error");
    } finally {
      setLoading(false);
    }
  ...
  const handleActivateConfirm = async () => {
    ...
    try {
      ...
      showToast(`Advisor "${getAdvisorName(activateModal)}" activated successfully ✅`);
  ...
  const handleSuspendConfirm = async () => {
    ...
    try {
      ...
      showToast(`Advisor "${getAdvisorName(suspendModal)}" suspended successfully ⚠️`);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showToast(`Advisor "${getAdvisorName(deleteModal)}" deleted successfully 🗑️`);
      setAdvisors(prev => prev.filter(a => getAdvisorId(a) !== id));
      setTotalAdvisors(prev => prev - 1);
      setDeleteModal(null);
    } catch (err) {
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, flex: 1 }}>
        <input
          ...
          style={{
            ...
            fontSize: 13, width: 240, outline: "none", transition: "all 0.2s"
          }}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F9FAFB", padding: "28px 24px" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#111827" }}>Vault Advisors</h1>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 380, padding: 28, boxShadow: "0 20px 60px rgba(0,0,0,0.15)", textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 22 }}>
              Are you sure you want to activate <strong>{getAdvisorName(activateModal)}</strong>? They will be granted access.
            </p>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 420, padding: 24, boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
              ...
              <h2 style={{ fontSize: 18, fontWeight: 700, color: "#111827" }}>Suspend Advisor</h2>
            </div>
            <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 16 }}>
              You are about to suspend <strong>{getAdvisorName(suspendModal)}</strong>. They will lose access until reactivated.
            </p>
      ...
      {deleteModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 380, padding: 28, boxShadow: "0 20px 60px rgba(0,0,0,0.15)", textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: "#6B7280", marginBottom: 22 }}>
              Are you sure you want to permanently delete <strong>{getAdvisorName(deleteModal)}</strong>? This action cannot be undone.
            </p>
            <div style={{ display: "flex", gap: 10 }}>
