...
import { BACKEND_URL, SITE_URL } from '@/config/urls';
...
export default function BlogPostPage({ blog, slug }) {
  ...
  let pageImage = blog.featuredImage || blog.coverImage || `${SITE_URL}/logoNew.png`;
  if (pageImage && !pageImage.startsWith('http://') && !pageImage.startsWith('https://')) {
    const separator = pageImage.startsWith('/') ? '' : '/';
    pageImage = `${BACKEND_URL}${separator}${pageImage}`;
  }
...
export async function getServerSideProps(context) {
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/blogs/get-all-blogs?isPublished=true&limit=100`);
    ...
    const detailRes = await fetch(`${BACKEND_URL}/blogs/get-blog-by-id?id=${blogId}`);
