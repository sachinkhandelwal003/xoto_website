...
const Avatar = ({ firstName, lastName, size = 36 }) => {
  const initials = `${firstName?.[0] || ""}${lastName?.[0] || ""}`.toUpperCase() || "XA";
  return (
    <div style={{
...
const AllAdvisors = () => {
  ...
  const [queryParams, setQueryParams] = useState({ page: 1, limit: 10, search: "", status: "", department: "" });

  // Suspend modal state
  const [suspendModal, setSuspendModal] = useState({ open: false, advisor: null, action: "suspend", reason: "" });
  ...
  // ── Fetch advisors ──
  const fetchAdvisors = useCallback(async (params = queryParams) => {
    setLoading(true);
try {
...
const paginationData = responseData?.pagination || { total: 0, page: 1, limit: 10, totalPages: 1 };
  ...
  notify("error", "Failed to load advisors", err?.response?.data?.message || "Something went wrong");
} finally {
      setLoading(false);
    }
  }, [queryParams]);
  ...
  // ── Server-side filter (from CustomTable) ──
  ...
  // ── Suspend / unsuspend ──
  const openSuspendModal = (advisor, action) => {
    setSuspendModal({ open: true, advisor, action, reason: "" });
  };
  ...
  const handleSuspendConfirm = async () => {
    ...
    setSuspending(true);
    try {
     await apiService.put(`/GridAdvisor/${advisor._id}/suspend`, {
  action,
  ...(action === "suspend" && { reason }),
      ...
      notify(
        ...
        `${advisor.firstName} ${advisor.lastName} has been ${action === "suspend" ? "suspended" : "reinstated"}.`
      );
      setSuspendModal({ open: false, advisor: null, action: "suspend", reason: "" });
      fetchAdvisors();
    } catch (err) {
  ...
  const stats = {
    ...
    inactive:   advisors.filter(a => a.status === "inactive" || a.status === "deactivated").length,
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      filterOptions: [
        { value: "Off-Plan Sales",   label: "Off-Plan Sales" },
      ...
      render: (val) => (
        <span style={{ fontSize: 12, color: val ? C.text : C.textMuted }}>
          {val || "Unassigned"}
        </span>
      ),
    },
      ...
      render: (_, row) => (
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button
            ...
            onClick={() => {
    const basePath = location.pathname.replace(/\/advisors$/, "");
    navigate(`${basePath}/advisors/${row._id}`);
  }}
          >
            <IconEye /> View
          </button>
  ...
  return (
    <>
      ...
      <div style={S.page} className="page-fade">
        <div style={S.container}>
          ...
          <div style={S.topBar}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                <IconAdvisors />
                <h1 style={S.heading}>Xoto Advisors</h1>
              </div>
          ...
          <div style={S.statsRow}>
            {[
              { label: "Total Advisors",   value: pagination.total, dot: C.primary   },
      ...
      <Modal
        ...
        width={460}
      >
        {suspendModal.advisor && (
          <div>
            <p style={{ color: C.textMuted, fontSize: 13, marginBottom: 16 }}>
              {suspendModal.action === "suspend"
                ? `You are about to suspend ${suspendModal.advisor.firstName} ${suspendModal.advisor.lastName} (${suspendModal.advisor.employeeId}). They will lose access immediately.`
                : `You are about to reinstate ${suspendModal.advisor.firstName} ${suspendModal.advisor.lastName}. They will regain full access.`
              }
            </p>
            {suspendModal.action === "suspend" && (
              <div>
