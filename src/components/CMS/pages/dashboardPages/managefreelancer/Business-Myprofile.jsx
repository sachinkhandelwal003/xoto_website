...
const MyprofileBusiness = () => {
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      await apiService.put('/business/change-password', passwordData);
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      await apiService.put(`/business/document/${updatingDocument}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      showToast('Document updated successfully', 'success');
      await fetchBusiness(); // Refresh profile
    } catch (error) {
  ...
  const getDocumentsArray = () => {
    ...
    return Object.keys(documentTypes)
      .map((type) => (business.documents[type] ? { ...business.documents[type], _id: business.documents[type]._id || type, type } : null))
      .filter(Boolean);
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === 'profile' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg transition-transform duration-300 hover:scale-[1.02]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900 font-medium">{business.store_details?.store_name || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Full Name</label>
                  <p className="text-gray-900 font-medium">{business.full_name || '--'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-gray-900 font-medium">
                    {business.mobile || '--'}{' '}
                    {business.is_mobile_verified && <span className="ml-2 text-xs text-green-600">Verified</span>}
                  </p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Store Type</label>
                  <p className="text-gray-900 font-medium">{business.store_details?.store_type || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Status</label>
                  <p
                    className={`inline-block px-3 py-1 text-xs font-medium rounded-full ${
                      business.status_info?.status === 0
                        ? 'bg-yellow-100 text-yellow-800'
                        : business.status_info?.status === 1
                        ? 'bg-green-100 text-green-800'
                        : 'bg-red-100 text-red-800'
                    }`}
                  >
                    {business.status_info?.status === 0 ? 'Pending' : business.status_info?.status === 1 ? 'Approved' : 'Rejected'}
                  </p>
                  {business.status_info?.status === 2 && business.status_info?.remarks && (
                    <p className="text-sm text-gray-600 mt-2">Remarks: {business.status_info.remarks}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            {/* Store Details */}
            <div className="bg-white p-6 rounded-xl shadow-lg transition-transform duration-300 hover:scale-[1.02]">
              <h2 className="text-xl font-semibold mb-4 text-gray-900">Store Details</h2>
              <div className="grid grid-cols-1 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-500">Store Address</label>
                  <p className="text-gray-900 font-medium">{business.store_details?.store_address || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Pincode</label>
                  <p className="text-gray-900 font-medium">{business.store_details?.pincode || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Tagline</label>
                  <p className="text-gray-900 font-medium">{business.store_details?.tagline || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Description</label>
                  <p className="text-gray-900 font-medium">{business.store_details?.store_description || '--'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg transition-transform duration-300 hover:scale-[1.02]">
              <h2 className="text-xl font-semibold mb-4 text-gray-900">Registration & Bank Details</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-500">PAN Number</label>
                  <p className="text-gray-900 font-medium">{business.registration?.pan_number || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">GSTIN</label>
                  <p className="text-gray-900 font-medium">{business.registration?.gstin || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Bank Account Number</label>
                  <p className="text-gray-900 font-medium">{business.bank_details?.bank_account_number || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">IFSC Code</label>
                  <p className="text-gray-900 font-medium">{business.bank_details?.ifsc_code || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Account Holder Name</label>
                  <p className="text-gray-900 font-medium">{business.bank_details?.account_holder_name || '--'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Preferred Currency</label>
                  <p className="text-gray-900 font-medium">{business.bank_details?.preferred_currency || '--'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg transition-transform duration-300 hover:scale-[1.02]">
              ...
              {business.store_details?.categories && business.store_details.categories.length > 0 ? (
                <div className="space-y-4">
                  {business.store_details.categories.map((category, index) => (
                    <div key={index} className="border-b border-gray-200 pb-4 last:border-0">
                      <h3 className="font-semibold text-gray-900">{category.name}</h3>
                      {category.subcategories && category.subcategories.length > 0 && (
                        <div className="mt-2">
                          <span className="text-sm text-gray-500">Subcategories: </span>
                          <span className="text-gray-700">{category.subcategories.join(', ')}</span>
                        </div>
                      )}
                    </div>
                  ))}
