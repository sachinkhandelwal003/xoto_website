...
import {
  ...
  Checkbox, Collapse, List, Image, Drawer, Empty, Statistic
} from 'antd';
import {
...
const BankProductViewWithDocuments = () => {
  ...
  const isAdmin = user?.role?.code === '18' || user?.role?.code === '0' || user?.role?.code === '1';
...
const downloadImage = async (imageUrl) => {
  ...
  const handleGoBack = () => {
    navigate(`/dashboard/${roleSlug}/bank/products`);
  };
  ...
  const customerDocs = documentsData?.customerDocuments || [];
  const bankDocs = documentsData?.bankForms || [];
    ...
    return (
      <Card
        ...
        style={{ 
          borderRadius: 16, 
          height: '100%',
          border: `1px solid ${doc.isMandatory ? '#ffccc7' : '#e8e8e8'}`,
          transition: 'all 0.3s'
        }}
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <Space>
            ...
            <Tooltip title="Preview">
              <Button type="text" icon={<EyeOutlined />} onClick={() => openPreview(doc)} />
            </Tooltip>
            <Tooltip title="Download">
<Button
  type="text"
  icon={<DownloadOutlined />}
  onClick={() => downloadImage(doc.fileUrl)}
/>            </Tooltip>
          </Space>
        </div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={12} md={6}>
          <Card style={{ borderRadius: 12, textAlign: 'center' }}>
            <Statistic title="Total Documents" value={stats.totalForms} prefix={<FileTextOutlined />} />
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card style={{ borderRadius: 12, textAlign: 'center' }}>
            <Statistic title="Mandatory" value={stats.mandatoryCount} valueStyle={{ color: '#ff4d4f' }} />
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card style={{ borderRadius: 12, textAlign: 'center' }}>
            <Statistic title="Customer Docs" value={stats.customerDocsCount} valueStyle={{ color: '#52c41a' }} />
          </Card>
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Card style={{ borderRadius: 12, textAlign: 'center' }}>
            <Statistic title="Bank Forms" value={stats.bankFormsCount} valueStyle={{ color: THEME_COLOR }} />
          </Card>
        </Col>
      </Row>
      ...
      <div style={{ display: activeTab === 'overview' ? 'block' : 'none' }}>
        <Row gutter={[24, 24]}>
          <Col xs={24} lg={16}>
            <Card title="Offer Summary" bordered={false} style={{ borderRadius: 16 }}>
              ...
              <Row gutter={[16, 16]}>
                ...
                <Col span={12}><Text type="secondary">Valid Until</Text><div>{product.offerSummary?.productValidity?.doesNotExpire ? <Tag color="green">Never Expires</Tag> : <Text>{dayjs(product.offerSummary?.productValidity?.expiryDate).format('DD MMM YYYY')}</Text>}</div></Col>
              </Row>
            </Card>
          ...
          <Col xs={24} lg={8}>
            ...
            {/* <Card title="Quick Actions" bordered={false} style={{ borderRadius: 16, marginTop: 16 }}>
              <Space direction="vertical" style={{ width: '100%' }}>
                <Button type="primary" block size="large" style={{ background: THEME_COLOR }}>Apply Now</Button>
                <Button block size="large">Calculate Eligibility</Button>
                <Button block size="large">Contact Advisor</Button>
              </Space>
            </Card> */}
          </Col>
        </Row>
          ...
          <Col xs={24} md={12}>
            <Card title="Loan Terms" bordered={false} style={{ borderRadius: 16 }}>
              <Descriptions column={1}>
                ...
                <Descriptions.Item label="Loan to Value (LTV)">{product.loanDetails?.loanToValue}% (Range: {product.loanDetails?.minLoanToValue}% - {product.loanDetails?.maxLoanToValue}%)</Descriptions.Item>
              ...
              <Row gutter={[16, 16]}>
                <Col xs={24} md={8}>
                  <div style={{ background: THEME_SECONDARY, padding: 16, borderRadius: 12, textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 24, fontWeight: 'bold' }}>AED {product.costBreakdown?.downPayment?.toLocaleString()}</div>
                    <Text type="secondary">({product.costBreakdown?.downPaymentPercentage}%)</Text>
                  </div>
                </Col>
                <Col xs={24} md={8}>
                  <div style={{ background: THEME_SECONDARY, padding: 16, borderRadius: 12, textAlign: 'center' }}>
                    <Text type="secondary">Total Upfront Cost</Text>
                    <div style={{ fontSize: 24, fontWeight: 'bold', color: THEME_COLOR }}>AED {product.costBreakdown?.totalUpfrontCost?.toLocaleString()}</div>
                  </div>
                </Col>
                <Col xs={24} md={8}>
                  <div style={{ background: THEME_SECONDARY, padding: 16, borderRadius: 12, textAlign: 'center' }}>
                    <Text type="secondary">Monthly EMI</Text>
                    <div style={{ fontSize: 24, fontWeight: 'bold' }}>AED {product.offerSummary?.monthlyEMI?.toLocaleString()}</div>
                  </div>
                </Col>
              </Row>
              <Divider />
              <Row gutter={[16, 16]}>
                ...
                <Col xs={24} md={12}>
                  ...
                  <Descriptions column={1} size="small">
                    ...
                    <Descriptions.Item label="Bank Pre-Approval Fee">AED {product.costBreakdown?.bankPreApprovalFee?.toLocaleString()}</Descriptions.Item>
          ...
          <Col xs={24} md={12}>
            <Card title="Eligibility Criteria" bordered={false} style={{ borderRadius: 16 }}>
              <Descriptions column={1}>
                ...
                <Descriptions.Item label="Minimum Experience">{product.eligibility?.minExperienceYears}+ years</Descriptions.Item>
                <Descriptions.Item label="Minimum Employment">{product.eligibility?.minEmploymentYears}+ years</Descriptions.Item>
        ...
        <Card style={{ borderRadius: 16, marginBottom: 16 }}>
          <Row gutter={16} align="middle">
            <Col xs={24} sm={8}>
              ...
              <Select value={filterSource} onChange={setFilterSource} style={{ width: '100%', marginTop: 8 }} size="middle">
            ...
            <Col xs={24} sm={8}>
              ...
              <Select value={filterType} onChange={setFilterType} style={{ width: '100%', marginTop: 8 }} size="middle">
        ...
        <Row gutter={[24, 24]}>
          <Col xs={24} md={12}>
            <Card title="Key Features" bordered={false} style={{ borderRadius: 16 }}>
              <Space direction="vertical" style={{ width: '100%' }}>
                {product.features?.keyFeatures?.map((feature, idx) => (
                  <div key={idx}><CheckCircleOutlined style={{ color: THEME_COLOR, marginRight: 8 }} />{feature}</div>
                ))}
              </Space>
            </Card>
          </Col>
          <Col xs={24} md={12}>
            <Card title="Benefits" bordered={false} style={{ borderRadius: 16 }}>
              <Space direction="vertical" style={{ width: '100%' }}>
                {product.features?.benefits?.map((benefit, idx) => (
                  <div key={idx}><StarOutlined style={{ color: THEME_COLOR, marginRight: 8 }} />{benefit}</div>
                ))}
              </Space>
            </Card>
          </Col>
          ...
          <Col xs={24}>
            <Card title="Disclaimers" bordered={false} style={{ borderRadius: 16 }}>
              {product.features?.disclaimers?.map((disclaimer, idx) => (
                <div key={idx} style={{ marginBottom: 8, color: '#ff4d4f' }}>⚠ {disclaimer}</div>
              ))}
            </Card>
          </Col>
        </Row>
...
<Drawer
  ...
  extra={
    <Button
      ...
      onClick={() =>
        selectedDoc?.fileUrl &&
        downloadImage(selectedDoc.fileUrl)
      }
      style={{
                ...
                {selectedDoc.fileUrl?.match(/\.(jpeg|jpg|png|gif)$/i) ? (
                  ...
                  <div style={{ padding: 40, textAlign: 'center' }}>
                    ...
                    <div style={{ marginTop: 16 }}>
                      <a href={selectedDoc.fileUrl} target="_blank" rel="noopener noreferrer">Open Document</a>
                    </div>
                  </div>
                ) : (
                  <div style={{ padding: 40, textAlign: 'center' }}>
          ...
          <Row gutter={16}>
            <Col span={24}>
              <Form.Item name="formName" label="Form Name" rules={[{ required: true }]}>
                <Input placeholder="e.g. Mortgage Application Form" size="large" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="formType" label="Form Type" rules={[{ required: true }]}>
                <Select size="large" placeholder="Select type">
                  ...
                  <Option value="disclosure_form">Disclosure Form</Option>
                  <Option value="noc_template">NOC Template</Option>
            ...
            <Col span={12}>
              <Form.Item name="formCategory" label="Category">
            ...
            <Col span={12}>
              <Form.Item name="documentSource" label="Document Source" initialValue="Customer">
                <Select size="large">
                  <Option value="Customer">Customer Document (Direct Upload)</Option>
                  <Option value="Bank">Bank Form (Download & Fill)</Option>
                </Select>
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="actionType" label="Action Type">
                <Select size="large">
                  ...
                  <Option value="download_fill_upload">Download → Fill → Upload</Option>
                </Select>
              </Form.Item>
            </Col>
            <Col span={24}>
              <Form.Item name="fileUrl" label="File URL">
                <Input placeholder="https://cdn.xoto.ae/forms/..." size="large" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="fileName" label="File Name">
                <Input placeholder="document-name.pdf" size="large" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="fileSize" label="File Size (bytes)">
                <InputNumber style={{ width: '100%' }} placeholder="325000" size="large" />
              </Form.Item>
            </Col>
            <Col span={12}>
            ...
            <Col span={8}>
              <Form.Item name="isMandatory" label="Is Mandatory?" valuePropName="checked" initialValue={true}>
                <Switch />
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item name="requiresSignature" label="Requires Signature?" valuePropName="checked" initialValue={true}>
                <Switch />
              </Form.Item>
            </Col>
            <Col span={8}>
              <Form.Item name="requiresAttestation" label="Requires Attestation?" valuePropName="checked" initialValue={false}>
                <Switch />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="applicableEmploymentTypes" label="Applicable Employment Types" initialValue={['Both']}>
                <Select mode="multiple" size="large" placeholder="Select types">
            ...
            <Col span={12}>
              <Form.Item name="applicableResidencyStatus" label="Applicable Residency Status" initialValue={['All']}>
                <Select mode="multiple" size="large" placeholder="Select status">
            ...
            <Col span={24}>
              <Form.Item name="description" label="Description">
                <TextArea rows={2} placeholder="Describe what this form is for..." />
              </Form.Item>
            </Col>
            <Col span={24}>
              <Form.Item name="fillInstructions" label="Fill Instructions">
                <TextArea rows={3} placeholder="Step by step instructions on how to fill this form..." />
              </Form.Item>
            </Col>
          </Row>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 16 }}>
            <Button onClick={() => { setIsAddModalOpen(false); addForm.resetFields(); }}>Cancel</Button>
            <Button type="primary" htmlType="submit" loading={addLoading} style={{ background: THEME_COLOR }}>
              Add Document
            </Button>
          </div>
        </Form>
