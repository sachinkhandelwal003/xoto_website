...
import {
  ...
  Activity, ArrowRight, RefreshCw, Briefcase, FileText, Eye, Info,
} from "lucide-react";
import {
  ...
  if (loading && !stats) {
    return (
      <div className="min-h-screen bg-[#f9f8ff] flex items-center justify-center flex-col gap-4">
        <Spin size="large" />
        <p style={{ color: P }} className="font-bold animate-pulse">Loading {agentTypeLabel} Earnings…</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#f9f8ff] p-5 sm:p-8">
      ...
      >
        ...
        <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="text-white/70 text-sm font-medium mb-1">{agentTypeLabel} Dashboard</div>
            <h1 className="text-2xl md:text-3xl font-black m-0 leading-tight">Your Earnings</h1>
            <p className="text-white/80 mt-2 text-sm font-medium max-w-md">
              Track your payouts, pending commissions, and successful disbursals as our {agentTypeLabel}.
            </p>
          </div>
          <Button 
      ...
      <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex items-center justify-between">
          <h3 className="text-lg font-black text-gray-800 m-0 flex items-center gap-2">
            <FileText size={20} className="text-purple-600" /> Latest Commission Records
          </h3>
        </div>
      ...
      >
        {selectedCommission && (
          <div className="py-4">
            ...
            {(selectedCommission.paymentReference || selectedCommission.paymentSentAt) && (
              <>
                <Divider className="my-6" />
                <div className="bg-gray-50 p-6 rounded-2xl">
                  <h4 className="flex items-center gap-2 font-black text-gray-800 mb-4"><TrendingUp size={16} className="text-blue-600" /> Payout Timeline</h4>
                  <Row gutter={24}>
                    <Col span={12}>
                      <div className="mb-4">
                        <div className="text-[10px] font-bold text-gray-400 uppercase mb-1">Transfer Reference</div>
                        <div className="font-bold text-gray-700 font-mono">{selectedCommission.paymentReference || "Processing..."}</div>
                      </div>
                    </Col>
                    <Col span={12}>
                      <div>
                        <div className="text-[10px] font-bold text-gray-400 uppercase mb-1">Payment Sent</div>
                        <div className="font-bold text-gray-700">
                          {selectedCommission.paymentSentAt ? dayjs(selectedCommission.paymentSentAt).format("DD MMM YYYY") : "Pending"}
                        </div>
                      </div>
                    </Col>
                  </Row>
