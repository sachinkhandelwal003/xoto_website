...
import {
  ...
  CodeOutlined, ApartmentOutlined, IdcardOutlined, PhoneOutlined,
  UserOutlined, MobileOutlined
...
const GRID_ROLES: VaultRole[] = [
  ...
  },
  {
    id: 'developer',
    label: 'Developer',
    description: 'Real estate developer portal',
    icon: <CodeOutlined />,
    color: '#F97316',
    gradient: 'linear-gradient(135deg, #F97316, #EA580C)',
    apiEndpoint: '/developer/login-developer',
    roleCode: '17',
  },
  {
    id: 'agency',
    label: 'Partner',
    description: 'Property partner company',
    icon: <ApartmentOutlined />,
    color: '#4F46E5',
    gradient: 'linear-gradient(135deg, #4F46E5, #4338ca)',
    apiEndpoint: '/agency/auth/login',
    roleCode: '15',
  },
  {
    id: 'agent',
    ...
    apiEndpoint: '/agent/login-agent',
    roleCode: '16',
  },
  {
    id: 'gridadvisor',
    label: 'Grid Advisor',
    description: 'Xoto internal grid advisor',
    icon: <UserOutlined />,
    color: '#0EA5E9',
    gradient: 'linear-gradient(135deg, #0EA5E9, #0284C7)',
    apiEndpoint: '/gridadvisor/login',
    roleCode: '24',
  },
  {
    id: 'gridreferralpartner',
    label: 'Referral Partner',
    description: 'Grid referral partner network',
    icon: <PhoneOutlined />,
    color: '#8B5CF6',
    gradient: 'linear-gradient(135deg, #8B5CF6, #7C3AED)',
    apiEndpoint: '/referral/login-partner',
    roleCode: '25',
  },
];

const FEATURES = [
  'End-to-end property listings',
  'Real-time lead tracking & updates',
      ...
      <div className="vl-wrap">
        ...
        >
          ...
          <div style={{ position:'relative',zIndex:2 }}>
            ...
            <motion.div initial={{ opacity:0,y:-16 }} animate={{ opacity:1,y:0 }} transition={{ duration:.7 }} style={{ marginBottom:8 }}>
              <img src="/KGT-RealEstate.png" alt="Xoto Grid"
            ...
            <motion.div initial={{ opacity:0,x:-24 }} animate={{ opacity:1,x:0 }} transition={{ duration:.7,delay:.15 }}>
              ...
              <div style={{ fontSize:14,color:'rgba(255,255,255,.55)',lineHeight:1.75,maxWidth:320,marginBottom:40 }}>
                Your complete platform for property listings, lead tracking, deals, and multi-role team collaboration.
              </div>
            </motion.div>
        ...
        >
          ...
          <div className="vl-mob-hdr">
            <img src="/KGT-RealEstate.png" alt="Xoto Grid"
              style={{ height:86,maxWidth:160,objectFit:'contain',filter:'brightness(1.2)' }}
              onError={(e) => { (e.target as HTMLImageElement).style.display='none'; }}
            />
                        ...
                        <Input
                          addonBefore={
                            <Select 
                              ...
                              onChange={(val) => { 
                                setCountryIso(val); 
                                form.setFieldsValue({ agent_phone: '' }); 
                              }}
                            >
                              {countryOptions.map((item) => (
                                <Select.Option key={item.iso} value={item.iso}>
                                  {item.name} (+{item.code})
                                </Select.Option>
                              ))}
                            </Select>
                          }
                          prefix={<MobileOutlined style={{ color:'#c4b5fd',fontSize:15 }} />}
