// ════════════════════════════════════════════════════════════════════════════
// PropertyDocumentLibrary.jsx — Admin: Manage documents for any property
// PRD §9.5 — Document Library (Admin view: full access to all docs)
// ════════════════════════════════════════════════════════════════════════════

import React, { useState, useEffect, useCallback } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { useSelector } from 'react-redux';
import { message, Select, Spin, Switch, Modal } from 'antd';
import {
  FiArrowLeft, FiUpload, FiTrash2, FiEdit3, FiFileText,
  FiEye, FiEyeOff, FiUsers, FiGlobe, FiSearch, FiFilter,
  FiDownload, FiX, FiCheck, FiPlus, FiFile, FiImage,
  FiRefreshCw,
} from 'react-icons/fi';
...
const FILE_TYPE_LABELS = { pdf: 'PDF', image: 'Image', other: 'Other' };
...
const CatPill = ({ cat }) => {
  ...
  return (
    <span style={{
      ...
      display: 'inline-block', letterSpacing: '0.3px', whiteSpace: 'nowrap',
    }}>
...
const UploadModal = ({ propertyId, onClose, onSuccess }) => {
  ...
  const handleFile = (f) => {
    ...
    setFile(f);
    setForm(prev => ({ ...prev, fileType: ft }));
  };

  const handleSubmit = async () => {
    if (!form.title.trim())          return message.error('Title is required');
    if (!form.documentCategory)      return message.error('Category is required');
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        margin: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.15)', overflow: 'hidden',
      }}>
        {/* Header */}
        <div style={{ background: GR, padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ margin: 0, color: 'rgba(255,255,255,0.8)', fontSize: 13 }}>Add document to property library</p>
          </div>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div
            ...
            style={{
              border: `2px dashed ${dragOver ? P2 : '#e2e8f0'}`,
          ...
          >
            <input id="doc-file-input" type="file" style={{ display: 'none' }} onChange={e => handleFile(e.target.files[0])} />
            {file ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              ...
              <select
                ...
                style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 10, padding: '10px 14px', fontSize: 14, outline: 'none', background: '#fff' }}
              >
          ...
          <div style={{ background: '#f8fafc', borderRadius: 12, padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: '#374151' }}>Visibility Settings</p>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <FiUsers size={16} color={P} />
                <div>
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 600 }}>Agent Visible</p>
                  <p style={{ margin: 0, fontSize: 11, color: '#64748b' }}>Grid Agents can see in catalogue</p>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 600 }}>Public</p>
                  <p style={{ margin: 0, fontSize: 11, color: '#64748b' }}>Registered customers see on listing</p>
                </div>
              </div>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            <button onClick={onClose} disabled={uploading} style={{ padding: '10px 20px', borderRadius: 10, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontSize: 14, fontWeight: 600, color: '#374151' }}>
              Cancel
            </button>
            <button onClick={handleSubmit} disabled={uploading} style={{ padding: '10px 24px', borderRadius: 10, border: 'none', background: GR, color: '#fff', cursor: uploading ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, opacity: uploading ? 0.7 : 1 }}>
              {uploading ? <><FiRefreshCw size={14} style={{ animation: 'spin 1s linear infinite' }} /> Uploading…</> : <><FiCheck size={14} /> Upload</>}
            </button>
          </div>
        </div>
      </div>
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <button onClick={handleSave} disabled={saving} style={{ padding: '10px 24px', borderRadius: 10, border: 'none', background: GR, color: '#fff', cursor: saving ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8, opacity: saving ? 0.7 : 1 }}>
              {saving ? <><FiRefreshCw size={14} /> Saving…</> : <><FiCheck size={14} /> Save</>}
            </button>
          </div>
        </div>
      </div>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function PropertyDocumentLibrary() {
  ...
  const handleDelete = async (docId) => {
    Modal.confirm({
      ...
      content: 'This will soft-delete the document. Are you sure?',
  ...
  const filtered = docs.filter(d => {
    ...
    const matchVis    = visFilter === 'all'
      || (visFilter === 'agent' && d.isAgentVisible)
      || (visFilter === 'public' && d.isPublic)
      || (visFilter === 'private' && !d.isAgentVisible && !d.isPublic);
    return matchSearch && matchCat && matchVis;
  });

  const stats = {
    total:   docs.length,
    agent:   docs.filter(d => d.isAgentVisible).length,
    public:  docs.filter(d => d.isPublic).length,
    private: docs.filter(d => !d.isAgentVisible && !d.isPublic).length,
  };
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: '#64748b' }}>Manage property documents, visibility & access control</p>
        </div>
        <button
          onClick={() => setShowUpload(true)}
          style={{ padding: '10px 20px', borderRadius: 12, border: 'none', background: GR, color: '#fff', cursor: 'pointer', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
        {[
          { label: 'Total',   val: stats.total,   bg: '#f3e8ff', color: P,        onClick: () => { setCatFilter('all'); setVisFilter('all'); } },
          { label: 'Agent',   val: stats.agent,   bg: '#e0f2fe', color: '#0369a1', onClick: () => setVisFilter('agent')  },
          { label: 'Public',  val: stats.public,  bg: '#dcfce7', color: '#166534', onClick: () => setVisFilter('public') },
          { label: 'Private', val: stats.private, bg: '#f1f5f9', color: '#475569', onClick: () => setVisFilter('private')},
        ].map(s => (
          <div key={s.label} onClick={s.onClick} style={{ background: s.bg, borderRadius: 14, padding: '16px 20px', cursor: 'pointer', transition: 'opacity 0.2s' }}>
            <p style={{ margin: 0, fontSize: 28, fontWeight: 800, color: s.color }}>{s.val}</p>
            <p style={{ margin: 0, fontSize: 12, color: s.color, opacity: 0.8, fontWeight: 600 }}>{s.label} Docs</p>
          </div>
        ))}
      </div>

      {/* ── Filters ── */}
      <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #f1f5f9', padding: '14px 16px', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 20 }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 180 }}>
          ...
          <input
            ...
            style={{ width: '100%', paddingLeft: 36, padding: '9px 12px 9px 34px', border: '1px solid #e2e8f0', borderRadius: 10, fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
          />
        </div>
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)} style={{ border: '1px solid #e2e8f0', borderRadius: 10, padding: '9px 14px', fontSize: 13, outline: 'none', background: '#fff', color: '#374151' }}>
          <option value="all">All Categories</option>
          {Object.entries(CATEGORY_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
        </select>
        <select value={visFilter} onChange={e => setVisFilter(e.target.value)} style={{ border: '1px solid #e2e8f0', borderRadius: 10, padding: '9px 14px', fontSize: 13, outline: 'none', background: '#fff', color: '#374151' }}>
          <option value="all">All Visibility</option>
          <option value="agent">Agent Visible</option>
          <option value="public">Public</option>
          <option value="private">Private</option>
        </select>
        <button onClick={load} style={{ padding: '9px 14px', borderRadius: 10, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#374151' }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 60, background: '#fff', borderRadius: 14, border: '1px solid #f1f5f9' }}>
          ...
          <p style={{ margin: 0, fontSize: 16, fontWeight: 600, color: '#94a3b8' }}>No documents found</p>
          <p style={{ margin: '6px 0 0', fontSize: 13, color: '#cbd5e1' }}>Upload a document to get started</p>
        </div>
      ) : (
            ...
            <div key={doc._id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 6, alignItems: 'center' }}>
                  ...
                  <VisBadge active={doc.isAgentVisible} label="Agent" icon={FiUsers} />
                  <VisBadge active={doc.isPublic} label="Public" icon={FiGlobe} />
                </div>
              </div>

              {/* Date */}
              <div style={{ textAlign: 'right', flexShrink: 0, display: 'none' }} className="doc-date">
                <p style={{ margin: 0, fontSize: 12, color: '#94a3b8' }}>{new Date(doc.createdAt).toLocaleDateString()}</p>
              </div>

              {/* Actions */}
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                <a href={doc.fileUrl} target="_blank" rel="noreferrer" style={{ width: 34, height: 34, borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b', textDecoration: 'none' }} title="View">
                  <FiDownload size={15} />
                </a>
      ...
      {editDoc && (
        <EditModal
          ...
          onSuccess={() => { setEditDoc(null); load(); }}
        />
      )}
    </div>
  );
