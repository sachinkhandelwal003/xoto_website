...
const AgentChip = ({ agent }) => {
  // agent is just an ID string — not populated by backend
  if (!agent || typeof agent === 'string') {
    return <span style={{ color: '#9ca3af', fontSize: 12 }}>— Not populated</span>;
  }
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <Avatar size={24} style={{ background: PRIMARY, fontSize: 10, fontWeight: 700 }}>
        {initials || '?'}
      </Avatar>
      <div>
...
const AllgridLeads = () => {
  ...
  const [filters, setFilters] = useState({}); // ← no more fixed agent filters
  ...
  const fetchLeads = useCallback(async (extraFilters = {}, page = 1, limit = 10) => {
    setLoading(true);
    try {
      const merged = { ...extraFilters }; // no forced lead_type or source
      ...
      if (merged.lead_type)         query.set('lead_type',      merged.lead_type);
      if (merged.source_channel)    query.set('source_channel', merged.source_channel);
  ...
  const statCards = [
    { label: 'Total Leads',    value: stats.total,      bg: PRIMARY + '15', color: PRIMARY,   icon: <TeamOutlined /> },
  ...
  return (
    <div style={{ padding: '28px 32px', background: '#faf5ff', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: '#111827' }}>All Leads</h1>
          <p style={{ margin: 0, fontSize: 13, color: '#6b7280' }}>
            Manage all platform leads — assign advisors and track progress
          </p>
        </div>
        <Button
          icon={<ReloadOutlined />}
      ...
      <div style={{
        ...
        {statCards.map((s, i) => (
          <div key={i} style={{
            background: '#fff', border: `1px solid ${PRIMARY}15`,
